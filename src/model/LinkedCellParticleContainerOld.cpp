...
#include "LinkedCellParticleContainerOld.h"
...
const void LinkedCellParticleContainer::initializeCells(std::array<BoundaryCondition, 6> &domainBoundaries)
{
    ...
    for (int i = 0; i < _numCells[0] * _numCells[1] * _numCells[2]; i++)
    {
        ...
        // halo cell
        if (index_3D[0] == 0 || index_3D[0] == _numCells[0] - 1 || index_3D[1] == 0 || index_3D[1] == _numCells[1] - 1 || index_3D[2] == 0 || index_3D[2] == _numCells[2] - 1)
        {
        ...
        _cellVector.emplace_back(type, cellBoundaries);
    }
...
const void LinkedCellParticleContainer::iterateParticles(std::function<void(Particle &)> f, bool calcX)
{
    ...
    for (auto &p : _activeParticleVector)
    {
        ...
        else if (new_cell_idx != p.getCellIdx())
        {
            ...
            {
                std::vector<int> crossedBoundaries;
                try
                {
                    crossedBoundaries = PContainer::crossedBoundary(p.getCellIdx(), new_cell_idx, _numCells);
                }
                ...
                // particle crossed periodic boundary in any direction, reappears on other side of domain
                bool wasPeriodic = false;
                for (int crossedBoundary : crossedBoundaries)
                {
                ...
                if (wasPeriodic)
                {
                    p.setX(mirroredPosition(p, crossedBoundaries));
    ...
    for (auto &cell : _cellVector)
    {
        if (cell.getType() == CellType::BoundaryCell)
        {
            for (int b = 0; b < 6; b++)
            {
                ...
                {
                    initGhostParticles(*cell.getCellParticles(), std::vector<int>{b});
                }
            }
        }
    }
...
std::array<double, 3> LinkedCellParticleContainer::mirroredPosition(Particle &p, std::vector<int> boundaries)
{
    ...
    for (int boundary_idx : boundaries)
    {
        ...
        case 0:
            newX = {p.getX()[0] + _domain[0], newX[1], newX[2]};
            break;
        ...
        case 2:
            newX = {newX[0], p.getX()[1] + _domain[1], newX[2]};
            break;
        ...
        case 4:
            newX = {newX[0], newX[1], p.getX()[2] + _domain[2]};
            break;
...
const void LinkedCellParticleContainer::initGhostParticles(std::vector<Particle *> &particles, std::vector<int> boundaries)
{
    ...
    {
        std::array<double, 3> newX = mirroredPosition(*p, boundaries);
